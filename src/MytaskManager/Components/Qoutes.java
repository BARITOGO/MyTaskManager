package MytaskManager.Components;

import java.util.ArrayList;
import java.util.List;
import java.util.Random;

public class Qoutes {
    private List<String> quotes;

    public Qoutes (){
            quotes = new ArrayList<>();
            
            quotes.add("Be yourself; everyone else is already taken.― Oscar Wilde");
            quotes.add("A room without books is like a body without a soul. ― Marcus Tullius Cicero");
            quotes.add("Be the change that you wish to see in the world. ― Mahatma Gandhi");
            quotes.add("If you tell the truth, you don't have to remember anything. ― Mark Twain");
            quotes.add("If you want to know what a man's like, take a good look at how he treats his inferiors, not his equals.― J.K. Rowling");
            quotes.add("Without music, life would be a mistake. ― Friedrich Nietzsche");
            quotes.add("Always forgive your enemies, nothing annoys them so much. ― Oscar Wilde");
            quotes.add("Life isn't about getting and having, it's about giving and being. –Kevin Kruse");
            quotes.add("Whatever the mind of man can conceive and believe, it can achieve. –Napoleon Hill");
            quotes.add("Strive not to be a success, but rather to be of value. –Albert Einstein");
            quotes.add("The greatest glory in living lies not in never falling, but in rising every time we fall. -Nelson Mandela");
            quotes.add("The way to get started is to quit talking and begin doing. -Walt Disney");
            quotes.add("The future belongs to those who believe in the beauty of their dreams. -Eleanor Roosevelt");
            quotes.add("If you set your goals ridiculously high and it's a failure, you will fail above everyone else's success. -James Cameron");
            quotes.add("You may say I'm a dreamer, but I'm not the only one. I hope someday you'll join us. And the world will live as one. -John Lennon");
            quotes.add("You must be the change you wish to see in the world. -Mahatma Gandhi");
            quotes.add("Spread love everywhere you go. Let no one ever come to you without leaving happier. -Mother Teresa");
            quotes.add(" Life is about making an impact, not making an income. -Kevin Kruse");
            quotes.add("Whatever the mind of man can conceive and believe, it can achieve. –Napoleon Hill");
            quotes.add("Strive not to be a success, but rather to be of value. –Albert Einstein");
            quotes.add("I attribute my success to this: I never gave or took any excuse. –Florence Nightingale");
            quotes.add("The most difficult thing is the decision to act, the rest is merely tenacity. –Amelia Earhart");
            quotes.add("Every strike brings me closer to the next home run. –Babe Ruth");
            quotes.add("Life isn't about getting and having, it's about giving and being. –Kevin Kruse");
            quotes.add("Life is what happens to you while you’re busy making other plans. –John Lennon");
            quotes.add("We become what we think about. –Earl Nightingale");
            quotes.add("The most common way people give up their power is by thinking they don’t have any. –Alice Walker");
            quotes.add("Don't worry when you are not recognized, but strive to be worthy of recognition. -Abraham Lincoln");
            quotes.add("Go confidently in the direction of your dreams! Live the life you've imagined. -Henry David Thoreau");
            quotes.add("Life is either a daring adventure or nothing. -Helen Keller");
            quotes.add("Keep smiling, because life is a beautiful thing and there's so much to smile about. -Marilyn Monroe");
            quotes.add("Your time is limited, so don't waste it living someone else's life. Don't be trapped by dogma — which is living with the results of other people's thinking. -Steve Jobs");
            quotes.add("I am not a product of my circumstances. I am a product of my decisions. –Stephen Covey");
            quotes.add("Every child is an artist.  The problem is how to remain an artist once he grows up. –Pablo Picasso");
            quotes.add("\"It is better to fail in originality than to succeed in imitation.\" -Herman Melville");
            quotes.add(" \"Always remember, your focus determines your reality.\" -George Lucas");
            quotes.add(" \"Try not to become a man of success. Rather become a man of value.\" -Albert Einstein");
            quotes.add("Remember that not getting what you want is sometimes a wonderful stroke of luck. –Dalai Lama");
            quotes.add(" \"If you want to achieve excellence, you can get there today. As of this second, quit doing less-than-excellent work.\" -Thomas J. Watson");
            quotes.add("\"If you genuinely want something, don't wait for it — teach yourself to be impatient.\" -Gurbaksh Chahal");
            quotes.add("\"The question isn't who is going to let me; it's who is going to stop me.\" -Ayn Rand");
            quotes.add("\"Beware of monotony; it's the mother of all the deadly sins.\" -Edith Wharton");
            quotes.add(" \"Nothing is really work unless you would rather be doing something else.\" -J.M. Barrie");
            quotes.add(" \"It's not just about being better. It's about being different. You need to give people a reason to choose your business.\" -Tom Abbott");
            quotes.add("\"Without hustle, talent will only carry you so far.\" -Gary Vaynerchuk");
            quotes.add(" \"Working hard for something we don't care about is called stressed; working hard for something we love is called passion.\" -Simon Sinek");
            quotes.add(" \"The key to life is accepting challenges. Once someone stops doing this, he's dead.\" -Bette Davis");
            quotes.add("\"Move out of your comfort zone. You can only grow if you are willing to feel awkward and uncomfortable when you try something new.\" -Brian Tracy");
            quotes.add(" \"Develop success from failures. Discouragement and failure are two of the surest stepping stones to success.\" -Dale Carnegie");
            quotes.add(" \"Formula for success: rise early, work hard, strike oil.\" -J. Paul Getty");
            quotes.add("\"Don't ask if your dream is crazy, ask if it's crazy enough.\" -Lena Waithe");
            quotes.add("\"If you cannot do great things, do small things in a great way.\" -Napoleon Hill");
            quotes.add("Learn from the mistakes of others. You can't live long enough to make them yourself. -Eleanor Roosevelt");
            quotes.add("Always do your best. What you plant now, you will harvest later. -Og Mandino");
            quotes.add(" \"Be patient with yourself. Self-growth is tender; it's holy ground. There's no greater investment.\" -Stephen Covey");
            quotes.add("Don't be distracted by criticism. Remember — the only taste of success some people get is to take a bite out of you. -Zig Ziglar");
            quotes.add("You will face many defeats in life, but never let yourself be defeated. -Maya Angelou");
            quotes.add("If life were predictable it would cease to be life and be without flavor. -Eleanor Roosevelt");
            quotes.add("An unexamined life is not worth living. –Socrates");
            quotes.add("Life is a succession of lessons which must be lived to be understood. -Ralph Waldo Emerson");
            quotes.add("The best time to plant a tree was 20 years ago. The second best time is now. –Chinese Proverb");
            quotes.add("The mind is everything. What you think you become.  –Buddha");
            quotes.add("Certain things catch your eye, but pursue only those that capture the heart. – Ancient Indian Proverb");
            quotes.add("Teach thy tongue to say, \"I do not know,\" and thous shalt progress. –Maimonides");
            quotes.add("Everything has beauty, but not everyone can see. –Confucius");
            quotes.add("When I let go of what I am, I become what I might be. –Lao Tzu");
            quotes.add("Happiness is not something readymade.  It comes from your own actions. –Dalai Lama");
            quotes.add("I didn’t fail the test. I just found 100 ways to do it wrong. –Benjamin Franklin");
            quotes.add("Life is trying things to see if they work. -Ray Bradbury");
            quotes.add("A truly rich man is one whose children run into his arms when his hands are empty. –Unknown");
            quotes.add("I would rather die of passion than of boredom. –Vincent van Gogh");
           
            };
    public  String getRandomQuote() {
        Random random = new Random();
        return quotes.get(random.nextInt(quotes.size()));
}
}
